...
def _contours_for_array(arr, transform, crs, levels, tile_id, index_interval=None):
    import matplotlib
    matplotlib.use("Agg")
    ...
    levels_tile = [float(v) for v in levels if zmin <= float(v) <= zmax]
    if not levels_tile:
        return ""

    nrows, ncols = arr.shape
    xs = np.zeros(ncols, dtype=float)
    ys = np.zeros(nrows, dtype=float)
    for c in range(ncols):
        xs[c], _ = xy(transform, 0, c, offset="center")
    for r in range(nrows):
        _, ys[r] = xy(transform, r, 0, offset="center")
    ...
    fig, ax = plt.subplots()
    try:
        cs = ax.contour(X, Y, arr, levels=levels_tile)
        parts = []
        for level, collection in zip(cs.levels, cs.collections):
            ...
            for path in collection.get_paths():
                ...
                if v is None or len(v) < 2:
                    continue
                coords = []
                step = max(1, int(len(v) / 2500))
                for x, y in v[::step]:
                    if to_wgs:
                        x, y = to_wgs.transform(float(x), float(y))
...
def generate_tiled_contours_from_dem(
    ...
    with rasterio.open(dem_path) as src:
        ...
        for i in range(tr):
            for j in range(tc):
                ...
                part = _contours_for_array(arr, wt, crs, levels, tile_id, index_interval=index_interval_m)
                if part:
    ...
    return TiledContourOutput(
        ...
        metadata={
            "modo": "curvas_por_teselas_unificadas",
